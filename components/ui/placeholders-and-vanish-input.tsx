...
export function PlaceholdersAndVanishInput({
  ...
  onChange,
  onSubmit,
}: PlaceholdersAndVanishInputProps): JSX.Element {
      ...
      requestAnimationFrame(() => {
        ...
        for (let i = 0; i < newDataRef.current.length; i++) {
          ...
          if (current.x < pos) {
            ...
            if (current.r !== undefined && current.r <= 0) {
              current.r = 0;
              continue;
            }
        ...
        if (ctx) {
          ...
          newDataRef.current.forEach((t) => {
            ...
            if (n > pos && typeof color === "string") {
              ctx.beginPath();
              ctx.rect(n, i, s ?? 1, s ?? 1);
              ctx.fillStyle = color;
